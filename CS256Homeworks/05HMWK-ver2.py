...
# Text blocks
purpose = (
    "This chart corrects a previous graphic that implied seniors (65–74 years) dominated SF "
    "Public Library checkouts. Summed totals for 2016–2024 show that 10–19 year-olds have the "
    "highest number of checkouts, with other age groups trailing."
)
source = (
    "San Francisco Public Library — Library Usage (DataSF)\n"
    "https://data.sfgov.org/Culture-and-Recreation/Library-Usage/qzz6-2jup/about_data"
)
method = (
    "Checkout records were grouped by year and summed by age range for 2016–2024. "
    "Bars are sorted high-to-low, drawn from a zero baseline, and labeled with exact totals to "
    "avoid inference from area/stacking. No category is visually emphasized."
)
data_ink_ratio = (
    "≈ 0.62 — About 62 percent of the visible elements represent data (bars, labels, and axes)."
)
tools = (
    "Python with pandas and Matplotlib libraries to aggregate, chart, and export "
    "a 1-megapixel PNG (1272×786)."
)
...
gs = gridspec.GridSpec(1, 2, width_ratios=[1.8, 1.6])
...
for bar in bars:
    width = bar.get_width()
    label_x = width * 0.02 + (width * 0.1 if width < 4_000_000 else 0)
    ax.text(
        label_x, bar.get_y() + bar.get_height()/2,
        f"{width:,}", va="center", ha="left", fontsize=12.5, color="white" if width < 5_000_000 else "black"
    )
...
# ---- Text Panel (auto layout, Option B) ----
...
plt.tight_layout()
plt.savefig("SFPL_Truth_Telling_Final_Balanced.png", dpi=110, bbox_inches="tight", pad_inches=0.15)
plt.close()
