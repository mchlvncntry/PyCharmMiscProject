#!/usr/bin/env python3
"""HW 11: BEFORE and AFTER contrast-adjusted histograms for an ASCII PGM image."""
...
def create_histogram(pixels, depth, bins=16):
    """Return (bucket_counts, bucket_size)."""
...
def tick_values_and_levels(max_count, height, ticks=10, tick_interval=None):
    """Return (tick_values, vertical_levels)."""
    if max_count <= 0:
...
def main():
    if len(sys.argv) != 2:
        print(f"\nUsage: python3 {sys.argv[0]} <path_to_pgm>\n", file=sys.stderr)
        sys.exit(1)
