    ...
    with open(file_path, "r", encoding="utf-8", errors="replace") as my_file:
        ...
        for hour, count in access_counts_by_hour(my_file):
            ...
            if previous_date and hour.date() != previous_date:
                print()
            print(f"{hour:%Y-%m-%d %H:%M:%S %z} {count:4d} "
                  f"access{'es' if count != 1 else ''}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        path = sys.argv[1] if len(sys.argv) > 1 else "/var/www/logs/access_log"
        err_type = "file not found" if isinstance(e, FileNotFoundError) else "no read permission for"
        print(f"Error: {err_type}: {path}")