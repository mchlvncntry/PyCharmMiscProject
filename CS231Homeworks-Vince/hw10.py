...
def plot_histogram(buckets, bucket_size, max_val, height=30):
    ...
    # centered bin labels 1 to 16
    label_row = "".join(f"{i+1:^{CELL}}" for i in range(num_buckets))
    print(LEFT + " " + label_row)

    caption_width = num_buckets * CELL  # 64 characters

    caption_text = (
        "Sixteen-bucket Histogram using only true linear scaling. "
        "Justification: I chose not to use logarithmic scaling because I did not want the resulting histogram to distort the actual data. "
        "Due to the large range of values, particularly in bucket 10, some smaller bars may not be visible at this scale."
    )

    wrapped = textwrap.fill(caption_text, width=caption_width)

    # Add left margin to match the bars
    ...
    for i in range(num_buckets):
        ...
        print(f"{LEFT} Bin {i+1:2d}: [{start:3d}-{end:3d}] = {buckets[i]:3,d} pixels")
    print()
